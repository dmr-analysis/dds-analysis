...
def my_parser(parser):
  ...
  optional.add_argument('--min_median_RPKM', help='Minimum median of RPKM value in each group, default=1.0, if it is smaller than 0 then this option will be switched off.',
  ...
  optional.add_argument('--sort_by_column_str', help='Column name of dataframe column that will be sorted after filtering , default=differential_expressoin_T_test_p_value if file is obtained from bpb3 package.', 
...
def filter_df_by_rratio(merged_df, rr_cutoff,min_median_RPKM, group1_str, group2_str):
 ...
 if min_median_RPKM>=0:
    ...
    print('No minumum median RPKM value is applied in filtering .')
    filtered_merged_df= merged_df[ (abs(merged_df.rratio)>=rr_cutoff)].copy()
...
def run(args):
  ...
  #tmp_columns2=[ os.path.basename(i.replace('_count','_count_zscoreLog_of')) for i in tmp_columns ]
  tmp_columns2=[]
  for ti in tmp_columns:
      if ti=='gene':
          ti2='gene_name'
      else:
          #jbw may
