...
#		2. predicted DMRs and map mutation blocks to chromSegment/genome are donne! in bpb3 and dmr_analysis, respectively
#
#This is a demo to predict target genes for DMR that overlapping in enhancers based on genCode annotation

### -- Parameters -- ####
##---INPUT PATH and files et al

# path of main folder input data
IN_FOLDER='../../data/fl_mr/in_data/'

#a) Path of mutation blocks and block summarys file exported by bpb3 package
...
#b) Path of exported DMRs/MRs from "dmr_analysis" 
...
#c) Path to a refFlat format gene annotation file that generated by dmr_gene_annotation
IN_GENOME_refFlat=${IN_GENOME_REGIONS}'/data/gencode.v19.annotation_gene.ref_clean_sorted.bed'

#d) Path of genome lib information files such as TAD,  chromatin segmentant, and genome files et al
IN_GENOME_LIB='../../data/'
...
#f) Path to gene positions in sorted bed format which is generated by hmst-seq-analyzer
...
IN_DMR=${IN_FOLDER}'/final_demo_data/fl_12samples/out_data/DMR_CpG_context/2_chroms_all_mr_data_range_dmrRanking.bed'

#i) Path to DEG genes exported from bpb3 package
IN_EXPRESS=${IN_FOLDER}'/bpb3_demo7_mr_enhancers/out/differentially_expressed_genes.txt'

#Try to add N bp flank region on the two sides of mutation blocks before intersecting them to DMRs
FLANK_BP=0
...
#3. map mutation blocks to DMR by adding flank regions in blocks if it is needed
...
#4. combine genomic regions with block information and find differential expressed genes
...
#10. rank gene list
#here tss=4, enhancer=3, tes=gene=2, 5dist=1
...
#12. pathway analysis
#this canbe done after DAVID analysis for the final gene list, if it is too larger after the filtering and gene ranking .
#OUT_FOLDER='out_blocks_dmr_single3_feb2022'
#dds_analysis go_pathway_analysis4out_blocks_gene -in_GEfile ${OUT_FOLDER}/out_dds/blocks_summary_block_position_0flank_0.7Proba_66868blocks_13143blocks2mr_4603blocks2dmr_deg_info_filtered_DMR_or_DEG_uniqGene_commonTAD_Boundary_list2UqGene_selectedGenes_gt_0.5.txt \
#        -in_GOfile ${OUT_FOLDER}/out_dds/DAVID_out_4Kdiff_DMR_or_DEG_mussd_signal3_weight4321_gt0.5.txt
